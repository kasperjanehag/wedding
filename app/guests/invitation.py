...
def send_invitation_email(party, test_only=False, recipients=None):
    ...
    if not recipients:
        print(
            "===== WARNING: no valid email addresses found for {} =====".format(
                party
            )
        )
        return
    ...
    print(
        "sending invitation to {} ({})".format(
            party.name, ", ".join(recipients)
        )
    )
    if not test_only:
        msg.send()
...
def send_all_invitations(test_only, mark_as_sent):
    to_send_to = (
        Party.in_default_order()
        .filter(is_invited=True, invitation_sent=None)
        .exclude(is_attending=False)
    )
