...
def update_code():
    with cd(env.code_root):
        ...
        sudo(
            "git reset --hard origin/%(code_branch)s" % env, user=env.sudo_user
        )
...
def update_virtualenv():
    """
    Update external dependencies on remote host assumes you've done a update.
    """
...
def touch_supervisor():
    ...
    """
    require("code_root", provided_by=ENVIRONMENTS)
    supervisor_path = posixpath.join(
        posixpath.join(env.services, "supervisor"), "supervisor.conf"
    )
